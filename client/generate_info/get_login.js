...
const url_2024 = `${BASE_URL}/v2/campus/31/users?per_page=150&filter[pool_year]=2024&filter[pool_month]=september`;
...
async function user_login(url) {
    ...
    do
    {
        ...
        activeUsers = data.filter((user) => !user["staff?"]);
...
(async () => {
    ...
    await writeLoginsToFile(users2024, 'aug.json');
})();
